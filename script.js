...
var ega = ["#000000", "#0000aa", "#00aa00", "#00aaaa",
"#aa0000", "#aa00aa", "#aa5500", "#aaaaaa", "#555555",
"#5555ff", "#55ff55", "#55ffff", "#ff5555", "#ff55ff",
"#ffff55", "#ffffff"];
var pallet = [];
var drawing= [];
    ...
    addEventListener('mousedown', (e) => {
      ...
      if (mouseX > this.x && mouseX < this.x + 100 && mouseY > this.y && mouseY < this.y + 100) {
        if (this.colorBit) {
          ...
          this.draw(context);
          //hier json schrijven naar server
          writeJson();
        }
      }
    })
...
function readJson(jsonString) {
  jsonObj = JSON.parse(jsonString);//string naar array-object
  for (i = 0; i < 64; i++) {
...
function writeJson(){
  var drawingExport = JSON.stringify(drawing);//van array_object naar string
  ajax("putJson", drawingExport);
}
...
function preload(){
  var drawingExport = JSON.stringify(preload);//van array_object naar string
  ajax("putJson", drawingExport);
}
